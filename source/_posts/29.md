...
看到 https://github.com/Infisical/infisical 的运行机制, 比较好奇它的运行机制. 然后发散性的想了一下, 如何将 https://github.com/hashicorp/vault 也变成一个同样的运行机制: 如何将它的秘钥注入到一个类似 python virtual env 中, 然后直接在这个环境中, 通过环境变量直接获取(和 `nix shell` 一样)
